    ...
    private suspend fun evaluateMatchingSkill(utterances: List<String>) {
        ...
        _state.value = _state.value.copy(
            pendingQuestion = PendingQuestion(
                ...
                continuesLastInteraction = skillRanker.hasAnyBatches(),
                skillBeingEvaluated = Pair(chosenSkill.skill.correspondingSkillInfo, isFallback),
            )
        )
        ...
        try {
            ...
            skillContext.previousInteractionWasFallback =
                _state.value.interactions.lastOrNull()?.skill?.second ?: false
