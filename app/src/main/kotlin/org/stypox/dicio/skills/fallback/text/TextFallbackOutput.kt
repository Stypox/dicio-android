...
class TextFallbackOutput(
    private val previousInteractionWasFallback: Boolean
) : HeadlineSpeechSkillOutput {
    override fun getSpeechOutput(ctx: SkillContext): String = ctx.getString(
        if (previousInteractionWasFallback) R.string.eval_no_match
        else R.string.eval_no_match_repeat
    )
    ...
    override fun getNextSkills(ctx: SkillContext): List<Skill<*>> =
        if (previousInteractionWasFallback) listOf() else listOf(
